...
public abstract class SaveServiceIT
{
	...
	protected List<Item> buildItems()
	{
		...
		AttributeUtil.addSourceAttribute(file, "http://example.com");
		AttributeUtil.addSourceDateTimeAttribute(file, new Date());
		TagUtil.addTag("Test Data", file);
		
		Host host = new Host();
		host.setHostName("malware.bad");
		
		AttributeUtil.addSourceAttribute(host, "http://somewebsite.com");
		AttributeUtil.addSourceDateTimeAttribute(host, new Date());
		...
		AttributeUtil.addSourceAttribute(threat, "http://lotsofthreats.bad");
		AttributeUtil.addSourceDateTimeAttribute(threat, new Date());
